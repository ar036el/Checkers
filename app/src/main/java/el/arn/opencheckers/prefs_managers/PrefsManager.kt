...
open class PrefsManager(
...
): HoldsDelegates<PrefsManager.Delegate> by delegationMgr {
    ...
    private fun registerPref(pref: Pref<*>) {
        _prefs.add(pref)
        delegationMgr.addDelegate(object : Delegate {
            override fun prefsHaveChanged(changedPref: Pref<*>) {
                if (changedPref == pref) {
                    (pref as PrefImpl).notifyDelegatesPrefHasChanged()
                }
            }
        })
...
interface Pref<V> : HoldsDelegates<Pref.Delegate<V>> {
    val key: String
...
abstract class PrefImpl<V> (
    ...
    protected val prefDelegationMgr: DelegatesManager<Pref.Delegate<V>> = DelegatesManager()
): Pref<V>, HoldsDelegates<Pref.Delegate<V>> by prefDelegationMgr {

    abstract override var value: V
    override fun restoreToDefault() {
        _value = defaultValue
    }
    fun notifyDelegatesPrefHasChanged() {
