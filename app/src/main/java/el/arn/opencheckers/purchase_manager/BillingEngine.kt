...
class BillingEngine(
...
) : HoldsDelegates<BillingEngine.Delegate> by delegationMgr {

    fun reloadPurchases(): Boolean /** @return [false] if the billing client is not connected. In that case, it tried to reconnect. Delegates will be notified when purchases will reload*/ {
    ...
    fun reloadPrices(): Boolean /** @return [false] if the billing client is not connected. In that case, it tried to reconnect. Delegates will be notified when purchases will reload*/ {
        if (billingClient.isReady) {
    ...
    private fun processPurchases(purchases: List<Purchase>) {
        CoroutineScope(Job() + Dispatchers.IO).launch {
            ...
            purchases.forEach { purchase ->
                var purchaseStatus: PurchaseStatus = PurchaseStatus.Unspecified
